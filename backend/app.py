...
qaadah_ankle = joblib.load('backend/qaadah/qaadah_ankle_LogisticRegression.pkl')
qaadah_hand = joblib.load('backend/qaadah/qaadah_hand_LogisticRegression.pkl')
qaadah_knee = joblib.load('backend/qaadah/qaadah_knee_LogisticRegression.pkl')


qiyam_hip = joblib.load('backend/qiyam/qiyam_hip_RandomForest.pkl')
ruku_hip = joblib.load('backend/ruku/random_forest_model_hip(2).joblib')
ruku_knee = joblib.load('backend/ruku/random_forest_model_knee(2).joblib')

sajda_ankle = joblib.load('backend/sajda/sajda_ankle_RandomForest.pkl')
sajda_hip = joblib.load('backend/sajda/sajda_hip_LogisticRegression.pkl')
sajda_knee = joblib.load('backend/sajda/sajda_knee_LogisticRegression.pkl')
...
#     ) as pose:
        ...
        # if video_name != "":
        #     output_video_path = f'C:/Users/syedn/OneDrive/Desktop/Islah.AI/backend/Output{video_name}'
...
def process_video():
    with mp_pose.Pose(
        ...
        # user_choice = int(
        #     input("Enter 0 to select video from system, 1 to record from webcam: ")
        # )
        # if user_choice == 0:
        #     video_path = select_video_from_system()
        ...
        if video_name != "":
            output_video_path = f'C:/Users/syedn/OneDrive/Desktop/Islah.AI/backend/Output{video_name}'
            # output_video_path = f'backend/Output{video_name}'
        ...
        while cap.isOpened():
            ...
            try:
                ...
                if hip_angle[0][0] > 150 and hip_angle[0][0] < 200:
                    ...
                    currentPose = "RUKU"
                    PostureText += currentPose
                elif hand_angle[0][0] >= 0 and hand_angle[0][0] <= 40 and knee_angle[0][0] > 0 and knee_angle[0][0] < 40 and ankle_angle[0][0] > 95 and ankle_angle[0][0] < 180:
                    currentPose = "QAADAH"
                    PostureText += currentPose
                elif hip_angle[0][0] >40 and hip_angle[0][0] < 110 and knee_angle[0][0] > 47 and knee_angle[0][0] < 112 and ankle_angle[0][0] > 95 and ankle_angle[0][0] < 180:
                    currentPose = "SAJDA"
                    PostureText += currentPose
                else:
                    currentPose = ""
                    PostureText = "incorrect posture"
                ...
                if currentPose == "QIYAAM":
                    ...
                    if knee_angle[0][0] > 180 and knee_angle[0][0] < 155 :
                        PostureText += "\nKnee too bent"
                
                    if ruku_hip.predict(hip_angle)[0] == 1 and knee_angle[0][0] <= 180 and knee_angle[0][0] >= 155:
                        PostureText += "\nGood"

                elif currentPose == "QAADAH":
                    if ankle_angle[0][0] > 173:
                        PostureText += "\nAnkle too straight"
                ...
                if(PostureText!="incorrect posture"):
                    frame = draw_selected_landmarks_on_image(
                        frame,
                        results.pose_landmarks,
                        selected_landmarks_indices,
